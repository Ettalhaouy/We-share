...
class Auth
{
    ...
    public function insertNewAnnounceData($db,$date,$session_id,$name_extension,$extensions_autorisation,$file_dest,$file_tmp_name){
        ...
        if (in_array($name_extension, $extensions_autorisation)) {
            if (move_uploaded_file($file_tmp_name, $file_dest)) {
                ...
                $db->query("INSERT INTO advertisements (title, photo, Description,date,id_organisaton)
    ...
    public function ModifyAnnounceData($db,$name_extension,$file_tmp_name,$file_dest,$extensions_autorisation,$date,$id){
        ...
        if (in_array($name_extension, $extensions_autorisation)) {
            if (move_uploaded_file($file_tmp_name, $file_dest)) {
                $db->query("UPDATE advertisements SET photo=?,date=? WHERE id=?", [$file_dest, $date, $id]);
                return true;
            }
        }
        return false;
    ...
    public function VirementVerification($db,$date,$id_ads,$amount,$name_extension,$extensions_autorisation,$file_dest,$file_tmp_name){
        
        if (in_array($name_extension, $extensions_autorisation)) {
            if (move_uploaded_file($file_tmp_name, $file_dest)) {

                $db->query("INSERT INTO donations (id_events, amount, Date) VALUES (?,?,?)", [$id_ads,$amount, $date]);
        
                $early_nb_donation_ads = $db->query('SELECT * FROM advertisements  WHERE id = ?', [$id_ads])->fetch();
            
                $new_nb_donation_ads = $early_nb_donation_ads->nb_Donation + $amount;
